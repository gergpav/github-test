...
# Добавьте эту проверку для yq
if ! command -v yq &>/dev/null; then
  echo "yq could not be found, please install it https://github.com/mikefarah/yq" >&2
  exit 1
fi
...
function create_environment() {
  ...
  for i in {1..30}; do
    ...
    if [ "$DB_STATUS" == "RUNNING" ]; then
      echo "Database is running, waiting additional 30 seconds for full initialization"
      sleep 30
      break
    fi
    if [ $i -eq 30 ]; then
      echo "ERROR: Database failed to become ready within 5 minutes"
      exit 1
    fi
    sleep 10
  done
